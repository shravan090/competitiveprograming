...
def isbinarytree(root, minimum, maximum):
    if root is None:
        return True

    if root.value < minimum or root.value > maximum :
        return False
    return (isbinarytree(root.left,minimum,root.value-1) and isbinarytree(root.right,root.value+1,maximum))
    
def is_binary_search_tree(root):
    # Determine if the tree is a valid binary search tree
    return isbinarytree(root,minimum,maximum)
